...
impl EventLoop {
    pub(crate) fn new(xconn: Arc<XConnection>) -> EventLoop {
        ...
        EventLoop {
            ...
            event_loop_proxy,
            state: EventLoopState { x11_readiness: Readiness::EMPTY, user_wake_up: false },
        }
    }
    ...
    fn single_iteration<A: ApplicationHandler>(&mut self, app: &mut A, cause: StartCause) {
        ...
        if mem::take(&mut self.state.user_wake_up) {
            app.user_wake_up(&self.event_processor.target);
        }
